...
def userregister(request):
    ...
    context['page_title'] = "Inscription des utilisateurs"
...
def update_profile(request):
    context = context_data(request)
    context['page_title'] = 'Modifier le profil'
    ...
    if not request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Votre compte a été modifié avec succès ! ")
...
def update_password(request):
    context =context_data(request)
    context['page_title'] = "Modifier le mot de passe"
    ...
    if request.POST:
        ...
        if user is not None:
            if user.is_active:
                ...
                resp['msg'] = "Mot de passe ou username erroné ! "
        else:
            resp['msg'] = "Mot de passe ou username erroné !"
    return HttpResponse(json.dumps(resp),content_type='application/json')
...
def save_user(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            if post['id'] == '':
                messages.success(request, "L'utilisateur a été sauvegardé avec succès !")
            else:
...
def delete_category(request, pk = None):
    resp = { 'status' : 'failed', 'msg':''}
    if pk is None:
        resp['msg'] = "L'identifiant de la catégorie est invalide"
    else:
        try:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if post['id'] == '':
                ...
                messages.success(request, "La sous-catégorie a été modifiée avec succès")
            resp['status'] = 'success'
        else:
            for field in form:
                for error in field.errors:
...
def save_book(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            if post['id'] == '':
                messages.success(request, "Le livre a été sauvegardé avec succès")
            else:
...
def save_student(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            if post['id'] == '':
                messages.success(request, "L'étudiant a été sauvegardé avec succès")
            else:
...
def borrows(request):
    context = context_data(request)
    context['page'] = 'borrow'
    context['page_title'] = "Liste des transactions d'emprunt"
...
def save_borrow(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            if post['id'] == '':
                messages.success(request, "La transaction d'emprunt a été enregistrée avec succès.")
            else:
                messages.success(request, "La transaction d'emprunt a été mise à jour avec succès.")
            resp['status'] = 'success'
        else:
            for field in form:
                for error in field.errors:
...
def view_borrow(request, pk = None):
    ...
    context['page_title'] = 'Visualiser les détails des transactions'
    if pk is None:
        context['borrow'] = {}
    else:
...
def manage_borrow(request, pk = None):
    ...
    context['page_title'] = 'Gérer les détails des transactions'
    if pk is None:
        context['borrow'] = {}
    else:
        context['borrow'] = models.Borrow.objects.get(id=pk)
...
def delete_borrow(request, pk = None):
    ...
    if pk is None:
        ...
        try:
            ...
            messages.success(request, "La transaction d'emprunt a été supprimée avec succès.")
            resp['status'] = 'success'
        except:
